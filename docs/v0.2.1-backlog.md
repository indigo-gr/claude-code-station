...
### [MEDIUM] npm-package distribution (`npm install -g claude-code-station`)
**File**: `package.json` / `install.sh`
**Issue**: The original HIGH ("install target cannot resolve better-sqlite3/yaml") is closed short-term — `install.sh` now symlinks the repo checkout's `node_modules` into the install dir. The remaining long-term work is option (b): a proper npm-package distribution with a `bin/` entry that resolves deps from its own install tree, removing the keep-the-checkout-around requirement.
**Fix sketch**: Publish to npm (or a GitHub tarball install) with `bin: { ccs: "./bin/ccs" }`; verify tsx resolution from the global install tree; deprecate the symlink path in install.sh.
**Why deferred**: Requires a publish decision (registry account, package name claim, release automation) — not just code.
